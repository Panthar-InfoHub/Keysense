...
export const createReservation = async (formData) => {
    try {
        ...
        // const whatsappResponse = await sendWhatsAppMessage(guest_id, formData.get("guest_no"), formData.get("guest_name"), formData.get("check_in"), formData.get("check_in"));
...
const sendWhatsAppMessage = async (reservationId, phoneNumber, guestName, checkIn, checkOut) => {
    try {
        const sensyApiUrl = "https://backend.aisensy.com/campaign/t1/api";
        const sensyApiKey = "YOUR_AI_SENSY_API_KEY";

        const reservationDashboardUrl = `https://hotels.keysense.co.in/HotelHariPalace/${reservationId}`;
        const formattedCheckIn = format(parseISO(checkIn), "yyyy-MM-dd");
        const formattedCheckOut = format(parseISO(checkOut), "yyyy-MM-dd");

        const whatsappPayload = {
            campaignName: "Hotel_Reservation_Confirmation",
            templateParams: [
                guestName,              
                reservationId,          
                phoneNumber,             
        ...
        const response = await axios.post(sensyApiUrl, whatsappPayload, {
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${sensyApiKey}`,
            },
        });
